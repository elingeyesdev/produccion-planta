...
@section('page_title', 'Editar Operador')
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('operadores.web.update', $operador->operator_id) }}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('first_name') is-invalid @enderror" 
                               id="first_name" name="first_name" value="{{ old('first_name', $operador->first_name) }}" required>
                        @error('first_name')
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('last_name') is-invalid @enderror" 
                               id="last_name" name="last_name" value="{{ old('last_name', $operador->last_name) }}" required>
                        @error('last_name')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('username') is-invalid @enderror" 
                               id="username" name="username" value="{{ old('username', $operador->username) }}" required>
                        @error('username')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" 
                               id="email" name="email" value="{{ old('email', $operador->email) }}">
                        @error('email')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('role_id') is-invalid @enderror" 
                            ...
                            @foreach($roles as $rol)
                                <option value="{{ $rol->role_id }}" {{ old('role_id', $operador->role_id) == $rol->role_id ? 'selected' : '' }}>
                                    {{ $rol->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="form-group">
                <label>
                    <input type="checkbox" name="active" value="1" {{ old('active', $operador->active) ? 'checked' : '' }}>
                    Activo
                </label>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" id="maquina_ids" name="maquina_ids[]" multiple>
                    @foreach($maquinas as $maquina)
                        <option value="{{ $maquina->machine_id }}" 
                                {{ in_array($maquina->machine_id, old('maquina_ids', $operador->machines->pluck('machine_id')->toArray())) ? 'selected' : '' }}>
